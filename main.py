...
### create domains
def split_domains(train_dst=None,factor=10,opts=None):
 
    subset_size = len(train_dst) // factor
    train_domains=[]
    for i in range(factor):
  
        start_idx = i * subset_size
        end_idx = (i + 1) * subset_size if i < factor - 1 else len(train_dst)
        print('id start',start_idx)
        print("id end ",end_idx)
        
        subset = Subset(train_dst, list(range(start_idx, end_idx)))
        print("subset",len(subset))
        subset_dataloader = DataLoader(subset, batch_size=opts.batch_size, shuffle=False)
        train_domains.append(subset_dataloader)
        print('subloader',len(subset_dataloader))
